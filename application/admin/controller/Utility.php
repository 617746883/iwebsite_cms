...
class Utility extends Controller
{
    ...
    public function selecturl()
    {
		...
		$columns = Db::name('column')->where('status',1)->field('id,name,parentid')->select();
    	$this->assign(['allUrls'=>$allUrls,'columns'=>$columns]);
    ...
    public function selecturlquery()
    {
		...
		if (!(empty($kw)) && !(empty($type))) {
			if ($type == 'content') {
				$list = Db::name('content')->where('status',1)->where('deleted',0)->where('title','like','%' . $kw . '%')->field('id,title')->select();
			}
		}
