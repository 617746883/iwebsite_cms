...
<div class="page-content">
    <form action="" method="get" class="form-horizontal form-search" role="form">
        <div class="page-toolbar">
            <div class="pull-left">
                <a class='btn btn-primary btn-sm' href="<?php echo url('admin/extend/banneradd'); ?>"><i class='fa fa-plus'></i> 添加幻灯片</a>
            </div>
            <div class="pull-right col-md-6">
                <div class="input-group">
                    <div class="input-group-select">
                        <select name="enabled" class='form-control'>
                            <option value="" <?php if($enabled == ''): ?> selected<?php endif; ?>>状态</option>
                            <option value="1" <?php if($enabled == '1'): ?> selected<?php endif; ?>>显示</option>
                            <option value="0" <?php if($enabled == '0'): ?> selected<?php endif; ?>>隐藏</option>
                        </select>
                    </div>
    ...
    <?php else: ?>
        <form action="" method="post">
            <div class="page-table-header">
                <input type='checkbox' />
                <div class="btn-group">
                    <button class="btn btn-default btn-sm btn-operation" type="button" data-toggle='batch' data-href="<?php echo url('admin/extend/bannerenabled',array('enabled'=>1)); ?>"><i class='icow icow-xianshi'></i> 显示</button>
                    <button class="btn btn-default btn-sm btn-operation" type="button" data-toggle='batch'  data-href="<?php echo url('admin/extend/bannerenabled',array('enabled'=>0)); ?>"><i class='icow icow-yincang'></i> 隐藏</button>
                    <button class="btn btn-default btn-sm btn-operation" type="button" data-toggle='batch-remove' data-confirm="确认要删除?" data-href="<?php echo url('admin/extend/bannerdelete'); ?>"><i class='icow icow-shanchu1'></i> 删除</button>
                </div>
            </div>
                ...
                <tbody>
                    <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$row): ?>
                        <tr>
                            ...
                            <td>
                                <a href='javascript:;' data-toggle='ajaxEdit' data-href="<?php echo url('admin/extend/bannerdisplayorder',array('id'=>$row['id'])); ?>" ><?php echo $row['displayorder']; ?></a>
                            </td>
                            <td><?php echo $row['bannername']; ?></td>
                            <td><?php echo $row['link']; ?></td>
                            <td>
                                <span class='label <?php if($row['enabled']==1): ?>label-primary<?php else: ?>label-default<?php endif; ?>'
                                        data-toggle='ajaxSwitch'
                                        data-switch-value='<?php echo $row['enabled']; ?>'
                                        data-switch-value0='0|隐藏|label label-default|<?php echo url('admin/extend/bannerenabled',array('enabled'=>1,'id'=>$row['id'])); ?>'
                                        data-switch-value1='1|显示|label label-primary|<?php echo url('admin/extend/bannerenabled',array('enabled'=>0,'id'=>$row['id'])); ?>' >
                                    <?php if($row['enabled']==1): ?>显示<?php else: ?>隐藏<?php endif; ?>
                                </span>
                            </td>
                            <td style="text-align:left;">
                                <a href="<?php echo url('admin/extend/banneredit', array('id' => $row['id'])); ?>" class="btn btn-op btn-operation" >
                                    <span data-toggle="tooltip" data-placement="top" data-original-title="修改">
                                        <i class='icow icow-bianji2'></i>
                                    </span>
                                </a>
                                <a data-toggle='ajaxRemove' href="<?php echo url('admin/extend/bannerdelete', array('id' => $row['id'])); ?>"class="btn btn-op btn-operation" data-confirm='确认要删除此幻灯片吗?'>
