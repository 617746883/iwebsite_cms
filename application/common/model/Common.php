...
class Common extends \think\Model
{
	...
	{
		$set = Db::name('sysset')->limit(1)->find();
	...
	{
		$setdata = Db::name('sysset')->field('id,sets')->limit(1)->find();

		if (empty($setdata)) {
			Db::name('sysset')->insert(array('sets' => iserializer($values)));
		}
		else {
			...
			Db::name('sysset')->where('id',$setdata['id'])->update(array('sets' => iserializer($sets)));
		}

		$setdata = Db::name('sysset')->limit(1)->find();
		return $setdata;
	}
	...
	{
		$setdata = Db::name('sysset')->field('id,plugins')->limit(1)->find();

		if (empty($setdata)) {
			Db::name('sysset')->insert(array('plugins' => iserializer($values)));
		}
		 else {
			...
			Db::name('sysset')->where('id',$setdata['id'])->update(array('plugins' => iserializer($plugins)));
		}
	...
	public static function getCopyright($ismanage = 1, $merchid = 0)
	{
		...
		if (!(is_array($copyright))) {
			$copyright = Db::name('system_copyright')->where('ismanage',$ismanage)->limit(1)->find();
		}
		$account = session('?account');
		if( !empty($merchid) ) 
		{
			$merch = Db::name('merch')->where('id',$merchid)->find();
			if( !empty($merch["logo"]) ) 
			{
    ...
    public static function getFullColumn($fullname = false, $status = false)
	{
		$allcategory = array();
		$sql = ' 1 ';
		...
		$category = Db::name('column')->where($sql)->order('parentid','asc')->order('sort','desc')->select();
