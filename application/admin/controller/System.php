...
class System extends Base
{
	...
	protected function adminpost()
	{
		...
		if (Request::instance()->isPost()) {
			...
			if (!empty($item['id'])) {
				...
				model('iwebsite')->plog('system.admin.edit', '编辑操作员 ID: ' . $id . ' 用户名: ' . $data['username'] . ' ');
			} else {
				...
				model('iwebsite')->plog('system.admin.add', '添加操作员 ID: ' . $id . ' 用户名: ' . $data['username'] . ' ');
			}
	...
	public function admindelete()
	{
		...
		foreach ($items as $item) {
			...
			model('iwebsite')->plog('system.admin.delete', '删除操作员 ID: ' . $item['id'] . ' 操作员名称: ' . $item['username'] . ' ');
		}
	...
	public function adminstatus()
	{
		...
		foreach ($items as $item) {
			...
			model('iwebsite')->plog('system.admin.edit', '修改操作员状态 ID: ' . $item['id'] . ' 操作员名称: ' . $item['username'] . ' 状态: ' . ($status == 0 ? '禁用' : '启用'));
		}
	...
	{
		$copyrights = Db::name('system_copyright')->where('ismanage',0)->find();
		...
		if (Request::instance()->isPost()) {
			...
			if(empty($copyrights)) {
				$id = Db::name('system_copyright')->insertGetId($data);
			} else {
				Db::name('system_copyright')->where('id',$copyrights['id'])->update($data);
			}
			show_json(1);
		}
	...
	{
		$copyrights = Db::name('system_copyright')->where('ismanage',1)->find();
	...
	public function rolestatus()
	{
		...
		foreach ($items as $item) {
			...
			model('iwebsite')->plog('perm.role.edit', '修改权限状态 ID: ' . $item['id'] . ' 角色名称: ' . $item['title'] . ' 状态: ' . ($status == 0 ? '禁用' : '启用'));
		}
	...
	public function roledelete()
	{
		...
		foreach ($items as $item) {
			...
			model('iwebsite')->plog('perm.role.delete', '删除权限 ID: ' . $item['id'] . ' 角色名称: ' . $item['title'] . ' ');
		}
	...
	public function permdelete()
	{
		...
		foreach ($items as $item) {
			...
			model('iwebsite')->plog('perm.role.delete', '删除角色 ID: ' . $item['id'] . ' 角色名称: ' . $item['title'] . ' ');
		}
	...
	public function permstatus()
	{
		...
		foreach ($items as $item) {
			...
			model('iwebsite')->plog('perm.role.edit', '修改角色状态 ID: ' . $item['id'] . ' 角色名称: ' . $item['title'] . ' 状态: ' . ($status == 0 ? '禁用' : '启用'));
		}
	...
	public function updatecache()
	{
		if (Request::instance()->isPost()) {			
            ...
            model('iwebsite')->plog('system.updatecache', '更新缓存');
	...
	public function database()
	{
		$do = input('do');
		$tab = input('tab');
		$dos = array('backup', 'restore', 'optimize','repair');
		...
		if ($do == 'backup') {
			...
			$iwebsiteset = model('common')->getSysset();
	        if($iwebsiteset['iwebsite']['close'] != 1) {
				...
				while (true) {
					...
					if (!empty($result)) {
						...
						if (strlen($dump) > $volumn) {
							...
							message('正在导出数据, 请不要关闭浏览器, 当前第 ' . $current_series . ' 卷.', url('admin/system/database',array('do'=>'backup','current'=>$current)), 'info');
						}						
					}
			...
			file_put_contents($bakfile, $dump);
			model('iwebsite')->plog('system.database.backup', '备份数据库，备份文件名：' . $folder_suffix);
			...
			if (!empty($delete_dirname)) {
				if(!empty($reduction[$delete_dirname]) && $this->system_database_backup_delete($delete_dirname)) {
					model('iwebsite')->plog('system.database.restore', '删除备份成功,' . $delete_dirname);
			...
			model('iwebsite')->plog('system.database.optimize', '数据库优化');
			...
			model('iwebsite')->plog('system.database.repair', '数据库修复');
			show_json(1,array('url' => url('admin/system/database',array('tab'=>'tab_repair'))));
		} elseif ($do == 'repair') {
			$select = input('select/a');
			foreach ($select as $tablename) {
				Db::query("REPAIR TABLE `{$tablename}`");
		...
		$this->assign(['list'=>array_map('array_change_key_case', $tables),'optimize_table'=>$optimize_table,'reduction'=>$reduction,'tab'=>$tab]);
