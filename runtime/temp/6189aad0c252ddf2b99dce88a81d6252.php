...
<div class="page-content">
   <form action="" method="get" class="form-horizontal" role="form">
        <div class="page-toolbar">
            <div class="col-sm-6">
                <span class="">
                    <a class='btn btn-primary btn-sm' href="<?php echo url('admin/column/add'); ?>"><i class="fa fa-plus"></i> 添加新栏目</a>
                </span>
            </div>			 
 ...
 <?php if(count($rules)>0): ?>
            <div class="page-table-header">
                <input type='checkbox' />
                <div class="btn-group">
                    <button class="btn btn-default btn-sm btn-operation" type="button" data-toggle='batch' data-href="<?php echo url('admin/column/status',array('status'=>1)); ?>">
                        <i class='icow icow-qiyong'></i> 启用</button>
                    <button class="btn btn-default btn-sm btn-operation" type="button" data-toggle='batch'  data-href="<?php echo url('admin/column/status',array('status'=>0)); ?>">
                        <i class='icow icow-jinyong'></i> 禁用</button>
                    <button class="btn btn-default btn-sm btn-operation" type="button" data-toggle='batch-remove' data-confirm="确认要删除?" data-href="<?php echo url('admin/column/delete'); ?>">
            ...
            <table class="table table-hover  table-responsive">
                ...
                <tbody>
                    <?php if(is_array($rules) || $rules instanceof \think\Collection || $rules instanceof \think\Paginator): if( count($rules)==0 ) : echo "" ;else: foreach($rules as $key=>$row): ?>
                    <tr>
                        <td><input type='checkbox' value="<?php echo $row['id']; ?>"/></td>
                        <td>
                            <a href='javascript:;' data-toggle='ajaxEdit' data-href="<?php echo url('admin/column/sort',array('id'=>$row['id'])); ?>" ><?php echo $row['sort']; ?></a>
                        </td>
                        ...
                        <td>
                           <span class='label <?php if($row['page']==1): ?>label-primary<?php else: ?>label-default<?php endif; ?>'
                                  data-toggle='ajaxSwitch' 
                                  data-switch-value='<?php echo $row['page']; ?>'
                                  data-switch-value0='0|禁用|label label-default|<?php echo url('admin/column/page',array('page'=>1,'id'=>$row['id'])); ?>'  
                                  data-switch-value1='1|启用|label label-success|<?php echo url('admin/column/page',array('page'=>0,'id'=>$row['id'])); ?>'  
                                >
                                  <?php if($row['status']==1): ?>启用<?php else: ?>禁用<?php endif; ?></span>
                        </td>                           
                        <td>
                           <span class='label <?php if($row['status']==1): ?>label-primary<?php else: ?>label-default<?php endif; ?>'
    							  data-toggle='ajaxSwitch' 
    							  data-switch-value='<?php echo $row['status']; ?>'
    							  data-switch-value0='0|禁用|label label-default|<?php echo url('admin/column/status',array('status'=>1,'id'=>$row['id'])); ?>'  
    							  data-switch-value1='1|启用|label label-success|<?php echo url('admin/column/status',array('status'=>0,'id'=>$row['id'])); ?>'  
    							>
    							  <?php if($row['status']==1): ?>启用<?php else: ?>禁用<?php endif; ?></span>
                        </td>
                        <td>
                            <a class='btn btn-default btn-sm btn-operation btn-op' href="<?php echo url('admin/column/add', array('parentid' => $row['id'])); ?>">
                             <span data-toggle="tooltip" data-placement="top" title="" data-original-title="添加子栏目">
                                <i class="icow icow-tianjia"></i>
                             </span>
                            </a>
                            <a class='btn btn-default btn-sm btn-operation btn-op' href="<?php echo url('admin/column/edit', array('id' => $row['id'])); ?>">
                             <span data-toggle="tooltip" data-placement="top" title="" data-original-title="编辑">
                                <i class="icow icow-bianji2"></i>
                             </span>
                            </a>
                            <a class='btn btn-default  btn-sm btn-operation btn-op' data-toggle="ajaxRemove"  href="<?php echo url('admin/column/delete', array('id' => $row['id'])); ?>" data-confirm="确认删除此栏目吗？">
